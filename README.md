...
- **抽牌 / 能量 / 疲劳**：首回合抽 5、之后抽 2；手牌上限 10 跳过；能量每回合重置为 4；
  疲劳递增（伤害 2/4/6…、生成「血战」攻击力 1/2/3…，敌我独立）。
- **弃牌堆（= 墓地）**：法术/攻击卡打出后进弃牌；仆从阵亡后进弃牌。
- **出牌**：仆从插入式召唤、直接攻击卡（打脸限制）、法术（含施法数连结算、仪式、冥界牵引选弃牌等）。
- **词条 / 场地**：嘲讽、护盾、重生、多重攻击、溅射、大型（占两格 + 属性增益 +50%）、
  仪式献祭（可多条并存、达标清零续积）、全局地狱场地（自动战斗后结算）、击杀抽牌等。
...
- **战斗 UI**：敌我角色区、7 格仆从区、手牌、弃牌选用、中线（能量/回合/结束回合/地狱与仪式摘要）、
  指向选目标 / 插入位置，Framer Motion 动画。
- **英雄与技能**：玩家**地狱术士**（灵魂汲取；击杀：抽取 1）；敌人**训练假人**（无技能）。
  默认卡组为地狱主题组（敌人不含地狱兽仪式）。
