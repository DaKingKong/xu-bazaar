...
- **词条 / 场地**：嘲讽、护盾、重生、多重攻击、溅射、大型（占两格 + 属性增益 +50%）、
  先锋（战斗开始置顶牌库）、仪式献祭（棋盘占位、永远在最右侧、献祭达标执行、生命为执行次数、归零进弃牌）、
...
- **战斗 UI**：敌我角色区、9 格仆从/仪式区（仪式固定右侧，多枚可区内重排）、手牌、弃牌选用、中线（能量/回合/结束回合/地狱摘要）、
...
- **内容配置**：`src/data/catalog.json` + 页头「配置」面板（搜索、逐卡/英雄 JSON、Save/导出/Reset；
  localStorage 按 `package.json` 版本门禁，刷新时载入一次）。
