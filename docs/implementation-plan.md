...
- [ ] `npm run dev` 可进行一局完整人机对战（出牌、选目标、选召唤位置、结束回合、自动战斗有动画）。
- [ ] `npm run build` 通过（类型检查无误）。
- [ ] `npm test` 全绿，覆盖 §2 全部规则。
- [ ] `src/engine` 为纯 TS，无 UI 依赖。
- [ ] 装备/遗物/技能/场地效果占位 UI 存在。
- [ ] README 完整，含结构、运行方式、待办与规格链接。
